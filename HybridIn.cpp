...
HybridInKnapsackSolver::HybridInKnapsackSolver(
        vector<Object> items,
int       capacity,
int       population_size,
int       iterations,
...
const HybridInKnapsackSolver::Solution_g& HybridInKnapsackSolver::solution() const
{
    return solution_;
}

auto HybridInKnapsackSolver::solve() -> decltype(solution())
{
    ...
    vector<Object> HybridInKnapsackSolver::getItems() {
    return items_;
}

bool HybridInKnapsackSolver::compare(const Object &a, const Object &b)
{
    return a.value / a.weight < b.value / b.weight;
}

unsigned HybridInKnapsackSolver::countBits(unsigned int number)
{
    // log function in base 2
    // take only integer part
    return (int)log2(number) + 1;
}
void HybridInKnapsackSolver::convertToBinary(int n)
{
    if (n / 2 != 0) {
        convertToBinary(n / 2);
    }
...
chromosome_t HybridInKnapsackSolver::generate_chromosome() const
{
...
int HybridInKnapsackSolver::evaluate(const chromosome_t& chromosome) const
{
...
const chromosome_t& HybridInKnapsackSolver::select(const vector<int>& fitnesses, int max) const
{
...
void HybridInKnapsackSolver::mutate(chromosome_t& chromosome)
{
...
chromosome_t HybridInKnapsackSolver::crossover(
...
int HybridInKnapsackSolver::weight(const chromosome_t& chromosome) const
{
    // Get value of all items or 0 if weight exceeds capacity
    int weight = 0;
...
std::vector<int> HybridInKnapsackSolver::getObjects()
{
    size_t weight = 0;
    size_t value = 0;
    int coef;
